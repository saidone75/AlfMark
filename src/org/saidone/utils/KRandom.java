...
public final class KRandom {
	...
	public static String randomizeString(int length) {
		...
		return randomizeString(length, set);
	}

	public static String randomizeString(int length, String set) {
		StringBuilder name = new StringBuilder();
		Random rnd = new Random(); 
		for (int i = 0; i < length; i++) {
			int key = Math.abs(rnd.nextInt() + 1) % set.length();
			name.append(set.charAt(key));
		}
